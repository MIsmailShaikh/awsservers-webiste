...
def daily_billing_check():
    with app.app_context():
        ...
        for vps in vps_instances:
            ...
            if datetime.now().date() >= gen_date_dt.date() and vps.last_billed_month != current_month:
                ...
                if user:
                    ...
                    nickname = vps.name
                    send_billing_email(user.company_name or user.username or f"Valued Customer", user.email, f"Cloud Server ({vps.vps_id})", nickname, vps.monthly_price * 1.18, gen_date_str, due_date_str)
        ...
        for ip in static_ips:
            if ip.monthly_price == 10.0 or ip.monthly_price == 13966.95:
                ip.monthly_price = 14285.52
                db.session.commit()
...
def force_trigger_billing():
    try:
        ...
        return f"SUCCESS! Billing email generated for IP {ip.address} (Base Price: {amount}, Total with 18% Tax: {total_with_tax}) and sent to {target_email} (User: {user.username})."
    except Exception as e:
        import traceback
